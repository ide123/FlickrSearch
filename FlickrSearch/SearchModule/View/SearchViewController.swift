...
class SearchViewController:UIViewController, UICollectionViewDelegateFlowLayout {
    ...
    override func viewDidLoad() {
        ...
        if let viewModel = (self.viewModel as? SearchableProtocol){
            ...
            _ = searchEventObs.withLatestFrom(searchTermObs).subscribe { [weak self] evt in
                if let term = evt.element {
                    ...
                    _ = viewModel.search(for: term!).bind(to:(self?.collectionView.rx.items)!){ cview,index,element in
                        let cell = (self?.collectionView.dequeueReusableCell(withReuseIdentifier: "imageCell", for: IndexPath(row:index,section:0)))! as! ImageCell
                    
                        let imgView = element.image
                        imgView!.contentMode = .scaleAspectFit
